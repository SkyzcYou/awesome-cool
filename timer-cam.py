...
import sys
import cv2
from util import detect_mask,detect_face
...
class Ui_MainWindow(QtWidgets.QWidget):
    ...
    def show_camera(self):
        ...
        ''' 调用 detect_mask() 方法判断是否戴口罩 '''
        isMask = detect_mask.detect_mask([show])
        # print(isMask)

        ''' 调用 detect_face() 方法识别人脸 '''
        # 将图像从BGR颜色（OpenCV使用）转换为RGB颜色（人脸识别使用）
        rgb_small_frame = show[:, :, ::-1]
        name = detect_face.get_detect_face(rgb_small_frame)
        print(name)


        show = cv2.cvtColor(show, cv2.COLOR_BGR2RGB)  # 视频色彩转换回RGB，这样才是现实的颜色 numpy.naday
